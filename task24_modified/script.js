document.addEventListener("DOMContentLoaded", () => {
  ...
  function sendInvitation(email, eventName, details) {
    if (validateEmail(email)) {
      ...
      if (!events[eventName]) {
        events[eventName] = { pending: 0, accepted: 0, declined: 0 };
      }
      events[eventName].pending++;
      ...
      invitations[eventName].forEach((invitation, index) => {
        ...
        acceptButton.addEventListener("click", () => {
          if (invitation.status === "Pending") {
            events[eventName].pending--; // Updates pending invitations
          }
  ...
  function updateResponseStats() {
    ...
    const responseRate =
      ((totalAccepted + totalDeclined) /
        (totalAccepted + totalDeclined + totalPending)) *
        100 || 0; // Avoid division by zero
    const acceptanceRate =
      (totalAccepted / (totalAccepted + totalDeclined)) * 100 || 0;

    responseStats.innerHTML = `
              <p>Sent Invitations: ${totalPending}</p>
  ...
  function updateEventStats() {
    ...
    Object.keys(events).forEach((eventName) => {
      ...
      eventDiv.innerHTML = `
                  <h2>${eventName}</h2>
                  <p>Sent Invitations: ${events[eventName].pending}</p>
